$(function()
{
  ...
  var month_start_date = set_month_year + '-' + set_month_month + '-' + set_month_day;
	...
	$('.date_text').click(function(){
		$('.wrapper').attr('data-id', $(this).parent()[0].id);
    ...
    $('.wrapper').attr('data-id',$(this).attr('id'));
		...
		// if($(this).attr('id') == 'datepicker_week'){
		// 	if(!!!val){
		// 		val = 'week';
		// 	}
		...
		// 	$('#dp-month').hide();
		// 	$('#dp-range').hide();
		// }else if($(this).attr('id') == 'datepicker3'){
		// 	if(!!!val){
		// 		val = 'day';
		// 	}
		...
		if(!!date1 && !!date2){
			...
			$(this).data('dateRangePicker').setDateRange(week_start_date,week_end_date);
		}
