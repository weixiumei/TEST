openerp.web.form = function (instance) {
  ...
  // 标志矩阵录入的数据 add by zxm: start
  var searchSkuData = false
  var get_change_default_count = 0
  // add by zxm: end
  ...
  //         });
      ...
      do_show: function (options) {
          var self = this;
          // 凡是有 tree 视图的全部隐藏分页 add by xcy
          if (this.ViewManager.views && this.ViewManager.views.tree) {
              $('.oe_view_manager_pager').css('display', 'none')
          }
          options = options || {};
          if (this.sidebar) {
      ...
      load_defaults: function () {
          ...
          if (keys.length) {
              return this.dataset.default_get(keys).then(function(r) {
                // add by zxm: start
              //   if (searchSkuData&&searchSkuData.item) {
              //     if (r&&r.hasOwnProperty('quantity')) {
              //         // 如果是 添加矩阵  则数量不取默认，而是自己加入
              //         r.quantity = searchSkuData.item.quantity
              //     }
              //   }
                // add by zxm: end
                self.trigger('load_record', r);
      ...
      init_pager: function() {
          ...
          this._init_bwizard_steps();
      },
      //add by xavi
      _init_bwizard_steps: function(){
          var form = $(".step-container");
          if(form.hasClass('stepInited')){
              return;
          }
          form.find('.step-section').hide();
          form.find('ol li').each(function(i){
              $(this).prepend('<span class="label badge">' + (i+1) + '</span>');
          });
          
          form.find('ol li').click(function(){
              ...
              var i = $(this).index()
              form.find('.step-section').fadeOut(0);
              form.find('.step-section').eq(i).fadeIn(300);
  
              $('.oe_steps_button_prev').prop('disabled',false);
              $('.oe_steps_button_next').prop('disabled',false);
              if(i == 0){
                  $('.oe_steps_button_prev').prop('disabled',true);
              }
              if(i == ($(this).siblings('li').size())){
                  $('.oe_steps_button_next').prop('disabled',true);
              }
          });
          
          var str = '';
          str += '<div class="bwizard-buttons">';
          str += '<button type="button" class="oe_button oe_steps_button_prev">上一步</button>';
          str += '<button type="button" class="oe_button oe_steps_button_next">下一步</button>';
          str += '</div>';
  
          form.append(str);
  
          $('.oe_steps_button_prev').click(function(){
              form.find('ol li.active').prev('li').click();
          })
          $('.oe_steps_button_next').click(function(){
              form.find('ol li.active').next('li').click();
          })
  
  
          form.find('ol li').eq(0).click();
          form.addClass('stepInited')
          form.addClass('clickable');
      },
      //add by xavi
      ...
      _process_onchange: function(on_change_obj) {
          ...
          try {
              ...
              return def.then(function(response) {
                  // wxm 2019/09/18 start
                  // console.log('response==', response)
                  // console.log('change_spec==', change_spec)
                  // console.log('batchInputProductInfo==', JSON.parse(sessionStorage.getItem('batchInputProductInfo')))
                  if(on_change){
                      get_change_default_count++
                  }
                  var batchInputProducts = JSON.parse(sessionStorage.getItem('batchInputProducts'))
                  if(!!batchInputProducts){
                      batchInputProducts.forEach(function(item, i){
                          if(!!change_spec && !!change_spec.args && item.item.id == change_spec.args[0]){
                              response.value.quantity = item.item.quantity
                          }
                      })
                      // 批量录入多条，覆盖完quantity，清缓存，防止影响下次录入数量
                      if(get_change_default_count==batchInputProducts.length){
                          sessionStorage.removeItem('batchInputProducts')
                          get_change_default_count = 0
                      }
                  }
                  // wxm 2019/09/18 end
      ...
      _process_save: function(save_obj) {
          ...
          try {
              ...
              if (form_invalid) {
                  ...
                  //add by xavi
                  //如果是bwizard steps结构的话，要显示第一个error的section
                  if(!!$(".step-container").size()){
                      var idx = first_invalid_field.$el.parents('section').index($(".step-container").children('section'));
                      $('.step-container ol li').eq(idx).click();
                  }
                  //add by xavi
  ...
  instance.web.form.FormRenderingEngine = instance.web.form.FormRenderingEngineInterface.extend({
      ...
      process_form: function($form) {
          if ($form.find('> sheet').length === 0) {
              $form.addClass('oe_form_nosheet position_relative');
          }
      ...
      */
      ...
      process_group: function($group) {
          ...
          if ($group.getAttributes().help){
                  this.do_attach_tooltip($group.getAttributes(), $new_group.children()[0]);
          }
          return $new_group;
      },
          ...
          options = _.extend({
                  ...
                  title: function() {
                      ...
                      if (!QWeb.has_template(template)) {
                          template = 'WidgetLabel.tooltip.group';
                      }
      ...
      process_label: function($label) {
          ...
          $label.before($new_label);
          $label.after(contents);
          $label.remove();
  ...
  */
  ...
  instance.web.form.WidgetButton = instance.web.form.FormWidget.extend({
      ...
      execute_action: function() {
          ...
          //add by xavi
          //2019-05-16
          else if (this.node.attrs.special == 'cancel') {
              self.view.reset = true;//设一个flag，以后reload的时候去做区分
              return exec_action();
          } 
          //end add by xavi
          else {
  ...
  instance.web.form.AbstractField = instance.web.form.FormWidget.extend(instance.web.form.FieldInterface, {
      ...
      renderElement: function() {
          ...
          } else if (this.$label.children() && this.$label.children()[0]) {
              this.do_attach_tooltip(this, this.$label.children()[0]);
          }
  ...
  */
  ...
  instance.web.form.FieldChar = instance.web.form.AbstractField.extend(instance.web.form.ReinitializeFieldMixin, {
      ...
      events: {
          'keyup input': 'delay_store_dom_value',
          'paste input': 'delay_store_dom_value',
      ...
      delay_store_dom_value: function(event){
          var self = this;
          self.last_time = event.timeStamp;
          setTimeout(function(){
              if(self.last_time - event.timeStamp == 0 && self.$('input').val() !== ''){
      ...
      is_syntax_valid: function() {
          if (!this.get("effective_readonly") && this.$("input").size() > 0) {
              try {
                  //针对正则做一个判断
                  //add by xavi
                  if(!!this.$('input').val() && !!this.$('input').attr('pattern')){
                      var reg = new RegExp(this.$('input').attr('pattern'),"g");
                      if(!reg.test(this.$('input').val())){
                          this.$('input').parent().addClass('oe_form_invalid');
                          return false;
                      }else{
                          this.$('input').parent().removeClass('oe_form_invalid');
                      }
                  }
  ...
  instance.web.form.FieldEmail = instance.web.form.FieldChar.extend({
      ...
      is_syntax_valid: function() {
          if (!!!this.$('input').attr('pattern')) {
              this.$('input').attr('pattern','^(\\w)+(\\.\\w+)*@(\\w)+((\\.\\w+)+)$');
          }
          return this._super();
      }
  });
  ...
  instance.web.form.FieldUrl = instance.web.form.FieldChar.extend({
      ...
      render_value: function() {
          if (!this.get("effective_readonly")) {
              ...
              if (this.node.attrs['filename'] == 'self' && this.get('value')){
                  text_list = this.get('value').split("/")
      ...
      is_syntax_valid: function() {
          if (!!!this.$('input').attr('pattern')) {
              this.$('input').attr('pattern','^(http|https)\:\/\/\w+\.\w*\.\w+\/?');
          }
          return this._super();
      }
  });
  ...
  instance.web.form.FieldFloat = instance.web.form.FieldChar.extend({
      ...
      is_syntax_valid: function() {
          //有控件不做默认校验
          if (!!!this.$('input').attr('pattern') && !!!this.widget) {
              var pattern = '';
              ...
              this.$('input').attr('pattern',pattern);
          }
          return this._super();
      }
  });
  ...
  instance.web.DateTimeWidget = instance.web.Widget.extend({
      ...
      start: function() {
          ...
          if(this.getParent().field){
              var field_type = this.getParent().field.type;
              if(field_type === 'datetime')
                self.$input.css({ width: '250px'})
              else if(field_type === 'date')
                self.$input.css({ width: '140px'})
          }
          ...
          this.$el.find('img.oe_datepicker_trigger').click(function() {
              ...
              self.picker('setDate', self.get('value') ? instance.web.auto_str_to_date(self.get('value')) : self.get_default_date());
      ...
      get_default_date: function(){
        if(this.getParent().node){
          var type_of_default_date = this.getParent().node.attrs.type;
          if(type_of_default_date === 'start')
            return instance.web.auto_str_to_date(instance.web.date_to_str(new Date()) + ' 00:00:00');
          else if(type_of_default_date === 'end')
            return instance.web.auto_str_to_date(instance.web.date_to_str(new Date()) + ' 23:59:59');
        }
        return new Date();
      },
      picker: function() {
  ...
  instance.web.form.FieldTextHtml = instance.web.form.AbstractField.extend(instance.web.form.ReinitializeFieldMixin, {
      ...
      initialize_content: function() {
          ...
          function nximgClick(e, data) {
      
              self.do_action({
                  name: _t('图片库'),
                  type: 'ir.actions.client',
                  tag: 'ck_gallery.action',
                  target: 'new',
                  flags: {
                      action_buttons: true,
                  },
          ...
          if (! this.get("effective_readonly")) {
              ...
              this.$textarea = this.$el.find('textarea');
              var width = ((this.node.attrs || {}).editor_width || '375px');
              var height = ((this.node.attrs || {}).editor_height || '667px');
              this.$textarea.cleditor({
                  ...
                  controls:   // controls to add to the toolbar
                              ...
                              " nximg source",
                  docCSSFile: "/web/static/src/css/cleditor-nexttao.css",
              ...
              $('.cleditorButton').removeClass('cleditorDisabled')
  ...
  */
          ...
          return this.orderer.add(dataset.name_search(
              ...
              if (values.length > self.limit) {
                  ...
                  values.push({
                      ...
                      action: function() {
                          //edit by songjian 2016-06-16 TODO 去掉调用数据已解决数据量过多出现的MemoryError,但此处理需要详细检验
  ...
  //                    },
  //                    classname: 'oe_m2o_dropdown_option fast_create'
  //                });
  //            }
  //            
  //            // create...
  ...
  //                },
  //                classname: 'oe_m2o_dropdown_option create_edit'
  //            });
  ...
  instance.web.form.FieldMany2One = instance.web.form.AbstractField.extend(instance.web.form.CompletionFieldMixin, instance.web.form.ReinitializeFieldMixin, {
      ...
      render_editable: function() {
          ...
          this.$drop_down.click(function(e) {
              console.log('drop_down.click')
              $('.ui-autocomplete').scrollUnique();
              ...
              if(e.shiftKey){
                $(self.$input.autocomplete('widget')).addClass("show_create_button");
              }else{
                $(self.$input.autocomplete('widget')).removeClass("show_create_button");
              }
          });
          ...
          this.$input.autocomplete({
              ...
              select: function(event, ui) {
                  ...
                  // by zhangxuemi ----- 用户体验 #38442
                  // 退换管理，换货明细录入商品，如果不回车的话，商品单价不会关联过来
                  ignore_blur = false
                  _this.$input.blur()
              },
              // add by zxm: start
              search: function(e, ui) {
                console.log('ui', ui)
                console.log('ui', JSON.parse(sessionStorage.getItem('searchSkuData')))
                // ui = JSON.parse(sessionStorage.getItem('searchSkuData'))
                if (!searchSkuData.item || $(e.target).parents('.oe_form_field_with_button').attr('data-fieldname') !== 'product_id') {
                  return
                }
                self.render_value();
                isSelecting = true;
                var item = ui.item;
              //   console.log('autocomplete触发搜索 =====>')
                item = searchSkuData.item
                if (item&&item.id) {
                    self.display_value = {};
                    self.display_value["" + item.id] = item.name;
                    self.reinit_value(item.id);
                    setTimeout(function(){
                      $('.oe_form_field_many2one[data-fieldname="product_id"] .ui-autocomplete-input:focus').blur()
                    }, 0)
                }
                sessionStorage.setItem('searchSkuData', '{}')
              },
              // add by zxm: end
              focus: function(e, ui) {
                  e.preventDefault();
      ...
      display_string: function(str) {
          ...
          if (!this.get("effective_readonly")) {
            ...
            if (! this.options.no_open)
                if (!! self.node.attrs.relativeurl){
                    $link.click(function () {
                    //   self.do_action({
                    //     name: '微信素材详情',
                    //     type: 'ir.actions.client',
                    //     tag: 'nt_wx.graphic_material_manage.action',
                    //     context: {
                    //       'group_code': 'GP1908011', 
                    //       'wechat_public_id': 35
                    //     }
                    //   })
                      var relativeurl = self.node.attrs.relativeurl ? eval('(' + self.node.attrs.relativeurl + ')') : {}
                      console.log('dnsahd>>>>', self, relativeurl)
                      var datarecord = self.view.datarecord
                      var context = {}
                      _.each(relativeurl.fields, function(data) {
                        context[data] = datarecord[data] instanceof Array ? datarecord[data][0] : datarecord[data]
                      })
                      // JSON.stringify(self.build_context().eval())
                      window.open('#action=' + decodeURIComponent(relativeurl.action) +  '&context=' + JSON.stringify(context), '_blank');
                      return false;
                    });
                }else{
                    $link.click(function () {
  ...
  */
  ...
  instance.web.form.One2ManyList = instance.web.ListView.List.extend({
      pad_table_to: function (count) {
          ...
          // add by wxm satrt
          var $searchProductDiv = $('<div>', {
            'class': 'oe_form_searchProductDiv'
          }).append(
            $('<input placeholder="请输入sku或者spu" class="searchProductInput" type="text"><span class="searchProDesc" style="display:none;color:red;"> 找不到商品</span>', {placeholder: '请输入sku或者spu'})
              // .blur(function(e) {
              //   e.preventDefault();
              //   e.stopPropagation();
              ...
              //     name: '批量录入',
              //     type: 'ir.actions.client',
              //     tag: 'nt_core.matrix_product_enter.action',
              //     target: 'new',
              //     context: {
              //       'name': 'zxm',
              //       'editorType': 'author'
              ...
              .keyup(function (e) {
                  ...
                  if(event.keyCode == 13) {
                      ...
                      var searchProductInput = document.getElementsByClassName('searchProductInput')[0].value
                      var adapter = new instance.web.Model('nt.product')
                      $.when(adapter.call('get_parent_id_by_code', [ {code: document.getElementsByClassName('searchProductInput')[0].value} ]))
                      .done((res) => {
                          // 返回列表 null是找不到商品， []是sku，非空列表是spu
                          $('.searchProDesc').hide()
                          console.log('get_parent_id_by_code', res)
                          if (res == null) {
                              $('.searchProDesc').show()
                          } else if (res.length == 0) {
                              document.getElementsByClassName('searchProductInput')[0].value = ''
                              var adapter = new instance.web.Model('nt.product')
                              $.when(adapter.call('h5_get_shop_product_detail', [ {product_code: searchProductInput} ]))
                              .done((res) => {
                                  console.log('sku===', res)
                                  var product = {
                                      item: {
                                          id: res.product.product_id,
                                          quantity: -1,
                                          name: res.product.product_name
                                      }
                                  }
                                  self.trigger_event(_this, [product], 0)
                              })
                          } else {
                              self.view.do_action({
                                  name: '批量录入',
                              ...
                              setTimeout(function(){
                                  $('.matrix_product_enter_main .confirm').click(function(){
                                      document.getElementsByClassName('searchProductInput')[0].value = ''
                                      var batchInputProductInfo = JSON.parse(sessionStorage.getItem('batchInputProductInfo'))
                                      // var productIds = [{
                                      //     item: {
                                      //         id: 32771,
                                      //         quantity: 2,
                                      ...
                                      // }]
                                      var products = []
                                      batchInputProductInfo.product_list.forEach(function(item, i){
                                          if(item.apply_qty>0){
                                              var product = {
                                                  item: {
                                                      id: item.product_id,
                                                      quantity: item.apply_qty,
                                      ...
                                      if(products.length>0){
                                          sessionStorage.setItem('batchInputProducts', JSON.stringify(products))
                                          self.trigger_event(_this, products, 0)
                                      }
          ...
          console.log('model==', self.view.model)
          // if (this.view.$el.find('table thead th[data-id="product_id"]').length > 0) {
          // #44202 增加DF 销售订单、损溢单 按SPU多选方式输入；
          if ('nt.shop.scrap.line' == self.view.model || 'nt.sale.order.line' == self.view.model) {
            // 判断是否有添加商品 td列
            if (!this.view.$el.parents('.oe_view_manager:first').find('.searchProductInput').length) {
              // 判断是否已经存在
              this.view.$el.parents('.oe_view_manager:first .oe_view_manager_body').before($searchProductDiv)
            }
          }
          // add by wxm end
      ...
      // add by zxm start
      trigger_event: function (_this_, productIds, index) {
        let _this = this
        $(_this_).parents('.oe_form_field_one2many:first').find('.oe_form_field_one2many_list_row_add a').trigger('click');
        // this.$current.find('.oe_form_field_one2many_list_row_add a').trigger('click');
        searchSkuData = productIds[index]
        setTimeout(function() {
          $('.oe_form_field_many2one[data-fieldname="product_id"] .ui-autocomplete-input').val(searchSkuData.item.name)
          console.log('daset=======trigger_event searchSkuData', searchSkuData)
          sessionStorage.setItem('searchSkuData', JSON.stringify(searchSkuData))
          $('.oe_form_field_many2one[data-fieldname="product_id"] .ui-autocomplete-input').autocomplete('search', searchSkuData);
        }, 100)
        // 批量 触发添加一个项目时，会有个异步的时间
        setTimeout(function() {
          $('.oe_form_field_many2one[data-fieldname="quantity"] .ui-autocomplete-input').val(searchSkuData.item.quantity)
          if (index+1 >= productIds.length) {
            // 标志矩阵录入 添加结束
            searchSkuData = {}
            return
          }
          _this.trigger_event(_this_, productIds, index+1)
        }, 500)
      }
      // add by zxm end
  });
  ...
  instance.web.form.FieldMany2ManyTags = instance.web.form.AbstractField.extend(instance.web.form.CompletionFieldMixin, instance.web.form.ReinitializeFieldMixin, {
      ...
      initialize_content: function() {
          ...
          //add by xavi
          //append remove all button
          var $clearAllBtn = $('<a href="javascript:void(0)" class="text-tags-remove-all">清空</a>');
          $clearAllBtn.click(function(){
              //edit by jiansong 直接清空即可
              self.set("value",[]);
          });
          self.$text.after($clearAllBtn);
          //add by xavi
              ...
              ext: {
                  ...
                  tags: {
                      ...
                      addTags: function(tags){
                          ...
                          return $.fn.textext.TextExtTags.prototype.addTags.apply(this, arguments);
                      },
      ...
      render_value: function() {
          ...
          if (! values || values.length > 0) {
            ...
            // add by zxm start #43022
            this._display_orderer.add(dataset.name_get(values)).done(handle_names);
            // add by zxm end #43022
              handle_names([]);
          }
      },
      add_id: function(id) {
  ...
  */
      ...
      setup_search_view: function(search_defaults) {
          ...
          this.searchview.on("search_view_loaded", self, function() {
              self.view_list = new instance.web.form.SelectCreateListView(self,
                      self.dataset, self.options.initial_view_id || false,
  ...
  instance.web.form.FieldBinaryFile = instance.web.form.FieldBinary.extend({
      ...
      on_clear: function() {
          ...
          this.set_filename('');
          // by zxm start (解决导入文件 清除文件以后，上传不了的问题)
          $("input.oe_form_binary_file").replaceWith('<input type="file" class="oe_form_binary_file" name="ufile">')
          this.$el.find('input.oe_form_binary_file').change(this.on_file_change)
          // by zxm end
      }
  });
  ...
  instance.web.form.FieldStatus = instance.web.form.AbstractField.extend({
      ...
      render_value: function() {
          ...
          if (color) {
              self.$(".oe_active .label").css("color", color);
          }
      },
