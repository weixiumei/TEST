<template>
  <div class="home">
    <!-- 创建的任务列表 -->
    <div class="weui-cells" v-show="isActive == 'now_task'" style="margin-bottom: 54px;">
          ...
          <div class="weui-tab__panel member_list">
            <div v-for="task in task_list" :key="task.task_code" style="border-bottom:1px solid #e5e5e5;">
              ...
              <div class="weui-cell">
                ...
                <div class="">
                  <p style="text-align: right;">完成进度：<span style="color:#ED9400;">{{task.market_percent}}%</span></p>
                </div>
                <!-- 指派给：{{task.task_user}} -->
              </div>
            </div>
      ...
      <!-- <navbar :items="state_list1" :selected="1"></navbar> -->
    </div>
    <!-- 任务详情 -->
    <div class="task_info" v-if="isActive == 'now_task_info'" style="padding-bottom: 1.6rem;min-height: 667px;background: #fff;">
      ...
      <div style="padding: 10px;">
        <div>
          ...
          <!-- <selectCell :options="state_list1" :selected="task_item.task_state"></selectCell> -->
      ...
      <div class="btn_div">
        <div v-on:click="goBack1()" class="weui-btn weui-btn_plain-primary">返回</div>
        <div v-on:click="edit_btn()" v-show="!edit" class="weui-btn weui-btn_plain-primary">编辑</div>
        <div v-on:click="save()" v-show="edit" class="weui-btn weui-btn_plain-primary" style="padding: 0 12px;margin-right: 0;">保存</div>
    ...
    <div v-show="isActive == 'member_list2'">
      <div class="members" :style="{maxHeight: maxheight + 'px','overflow':'scroll','background':'#fff'}">
         <!-- v-on:click="goMemberDetail(member)" -->
         <!-- href="{WX_URL_PREFIX}+'/nt-html-pbmini/nt-member-phone/?token='+{WPDF_TOKEN}+'&member_code='+{member.member_code}+'&member_no='+{member.member_code}+'#/'" -->
        <div v-for="( member,index) in member_list" :key="member.code" style="border-bottom: 4px solid #EFEFF4;padding-bottom: 6px;">
          <div class="weui-cell">
            <a v-on:click="goMemberDetail(member)" class="weui-cell__bd" :href="WX_URL_PREFIX+'/nt-html-pbmini/nt-member-phone/?token='+WPDF_TOKEN+'&member_code='+member.member_code+'&member_no='+member.member_code+'#/'">
          ...
          <!-- <checkboxCell :id="11" :name="checkbox1" :value="11" :label="11" :checked="true"></checkboxCell> -->
          <!-- <radioCell :value="未执行促销" :picked="member.mobile"></radioCell> -->
          <!-- <input type="radio" :name="name" class="weui_check" :id="id" :value="未执行促销" v-model="member.selected">
          <span class="weui_icon_checked">3456</span> -->

        </div>
      </div>
      <div class="btn_div">
        <div v-on:click="goBack2()" class="weui-btn weui-btn_plain-primary">返回</div>
      </div>
    </div>
    <!-- 筛选会员 -->
    <div v-show="isActive == 'create'" style="padding-bottom: 60px;">
      ...
      <!-- <i v-show="member_list_loading" class="weui-loading"></i> -->
		  <div v-on:click="next_member_list()" class="weui-btn weui-btn_plain-primary weui-btn_loading" style="margin: 15px 55px 0 55px;">下一步</div>
    </div>
    <!-- 筛选出的会员list -->
    <div v-show="isActive == 'member_list'">
      <div v-if="member_list.length>0">
        ...
        <div class="btn_div">
          ...
          <div v-on:click="next_to_edit()" class="weui-btn weui-btn_plain-primary">下一步</div>
        </div>
      </div>
      <div v-if="member_list.length==0">
    ...
    <!-- 编辑任务 -->
    <div class="task_info" v-if="isActive == 'task_edit'" style="padding-bottom: 1.6rem;min-height: 667px;background: #fff;">
      <!-- <input type="text" style="border:1px solid #d5d5d5;" v-model="task_item.task_name"> -->
      <div style="padding: 10px;margin-top: 0;">
      ...
      <div class="btn_div">
        ...
        <div v-on:click="finish()" class="weui-btn weui-btn_plain-primary">完成</div>
      </div>
    </div>
...
<script>
  ...
  // import selectCell from 'vue-weui/components/cells/cell-select'
  // import radioCell from 'vue-weui/components/cells/radio-cell'
  // import checkboxCell from './../../../node_modules/components/cells/checkbox-cell'
  import { GET_SHOP_LIST, MARKETING_TASK_LIST, GET_MARKETING_TASK_INFO, MEMBER_LEVEL_LIST, MEMBER_LIST, MARKETING_TASK_EDIT_INFO, GET_SEARCH_RELATED_USERS, UPDATE_TASK_INFO, UPDATE_MEMBER_INFO } from "@/api";
  export default {
    name: "home",
    data() {
      return {
    ...
    methods: {
      ...
      goMemberDetail: function (e) {
        ...
        // window.location.href = WX_URL_PREFIX + '/nt-html-pbmini/nt-member-phone/?token='+`${window.localStorage.getItem('WPDF_TOKEN')}`+'&member_code='+e.member_code+'&member_no='+e.member_code+'#/'
      },
      // 下一步到会员list
      ...
      goTaskDetail: function (e) {
        ...
        //   case 'draft':
        //     this.task_item.task_state_text = '未开始';
        //     break;
        //   case 'process':
      ...
      next_to_edit: function () {
        ...
        // this.member_selected_list = JSON.stringify(JSON.parse(this.member_list));
        ...
        console.log(this.member_selected_list,this.member_list)
        this.loadingToastShow = true;
        let that = this;
        // 导购列表
    ...
    mounted: function() {
      ...
      this.maxheight = screabHeight - 72;
    },
    created(){
      // 任务列表
      this.get_marketing_task_list();
